...
import EntityExtractor from '../libs/entity-extractor';
import utils from '../libs/ExtraFunctions';
...
export class Agent extends EventEmitter {
  public isAgentBuilding: boolean;
  public modelpath: string;
  public words: any[];
  public classes: any[];
  public documents: any[];
  public ignore_words: string[];
  public context: Map<any, any>;
  public intents: any[];
  public synonyms: any[];
  ...
  public constructor(language: string, debug = false) {
    super();
    ...
    this.modelpath = path.join(globals.RootPath(), 'trained-models');
    //train arrays
    this.words = [];
    this.classes = [];
    this.documents = [];
  ...
  _createEntityExtractor(language: string) {
  ...
  _setContext(userid: string, contextText: any) {
    const userContext = this._getContext(userid) ?? [];
    userContext.push(contextText);
    this.context.set(userid, utils.unique(userContext));
  }

  _getContext(userid: string) {
    return this.context.get(userid);
  }

  _configResponse(sentence: string) {
    const replaceAll = (str: any, needle: any, replacement: any) => {
  ...
  _createModel(trainXSize: any, trainYSize: any) {
  ...
  _emitAndGetFallback(sentence: string, response: any, userID: string) {
    const fallbackMsg = utils.random(this._getFallBack());
    if (this._debug) console.log(`Registering fallback to user ${userID}`);
    this.emit(
      'conversation',
      [
        {
  ...
  _saveConversation(sentence: string, userResponse: any, tag: any, confidence: any, userID: string) {
    this.emit(
      'conversation',
      [
        {
  ...
  trainBuilder() {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        //train model
        const abc = await model.fit(xs, ys, {
          epochs: 300,
          batchSize: 8,
          shuffle: true,
          // verbose: 1,
      ...
      try {
        ...
        for (let i = 0; i < iMax; i++) {
          ...
          for (let j = 0; j < jMax; j++) {
            ...
            const wd = this.tokenizer.tokenize(String(intentsList[i].patterns[j]));
            ...
            let steamWords = wd.map(word => natural.PorterStemmer.stem(word.toLowerCase()));
  ...
  async clean_up_sentence(sentence: string) {
    //stem and tokenize the pattern
    const sentence_words = this.tokenizer.tokenize(sentence);
    //fix words
    ...
    for (let i = 0; i < iMax; i++) {
      ...
      for (let j = 0; j < jMax; j++) {
        //synonym
        sentence_words[i] = natural.PorterStemmer.stem(sentence_words[i].toLowerCase());
        const xMax = synonym[j].synonyms.length;
        for (let x = 0; x < xMax; x++) {
          //synonym list
          if (String(synonym[j].synonyms[x]).toLowerCase() === sentence_words[i]) {
  ...
  classify(sentence: string, catchGuess = false) {
    return new Promise(async (resolve, reject) => {
      ...
      const guesses_list: any[] = [];
      //test if the BowData is a array of zeros (If enable)
      const NotAllZeros = catchGuess ? true : await utils.zeroTest(bowData);
      ...
      if (NotAllZeros) {
        ...
        tf.tidy(() => {
          ...
          const predictions = (this.model.predict(data) as Tensor).dataSync();
          ...
          predictions.forEach((prediction, index) => {
            if (prediction > this.CONFIDENCE) {
              ...
              if (prediction > this.CONFIDENCE / 2) {
                //to be a guesses should be half of the current confidence
  ...
  response(sentence: string, userID: string) {
    return new Promise(async (resolve, reject) => {
      try {
        const responses: any = await this.classify(sentence);
          ...
          for (let j = 0; j < jMax; j++) {
            ...
            if (this.intents[j].tag == results[i][0]) {
              ...
              if (intentHasContextSet) {
                ...
                if (this._debug) console.log('set context: ' + this.intents[j]['context_set']);
              }
              ...
              if (!contextFilterInIntents || (contextFilterInIntents && userContextHasFilter)) {
                const userResponse = this._configResponse(this.intents[j]['responses']);
                if (!intentHasContextSet && userContextHasFilter) {
                  if (this._debug) {
                    console.log('removing context: ' + this.intents[j]['context_filter']);
                  }
